model AZEAT_Indi_Engine_Control_Test_3
  //=============
  ...
  // External Model Repository for Local AZEAT_Indi_Engine_Control_Test_3 Use
 ...
 Modelica.Blocks.Interfaces.RealInput u annotation(
      Placement(transformation(origin = {-106, 10}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-106, 10}, extent = {{-20, -20}, {20, 20}})));
 Modelica.Blocks.Interfaces.RealOutput y annotation(
  ...
  algorithm
    when initial() then
      area := 0; tPrev := time; uPrev := max(0,u);
    end when;
  ...
  annotation(
    ...
    model IndividualControllerCore5
      ...
      "Per-gen allocator with efficient target pEff and richer battery participation"
    ...
    // If not last: cap at pMax or pHi? – your policy said hold to pMax when last, pHi otherwise
          if isLast then
            if useBattery then
              ...
              yCand := pMax; // alternative: demand (then unmet handled outside)
              tempNode := 4.1;
            end if;
          else
            if useBattery then
               if noEvent(cap >0) then
                  yCand := pHi;   // non-last keeps pHi and passes the res
                  tempNode := 4.2;
               else
                 yCand := pMax;
                 tempNode:=4.21; 
               ...
               yCand := pMax;
               tempNode := 4.3;
             end if;   // non-last keeps pHi and passes the rest
          end if;
      end if;
    ...
    // SoC hysteresis for "needs charging"
      when initial() or SOC_1 <= SOC_min or SOC_1 >= SOC_max then
        batNeedsChargingR := 
                      if SOC_1 <= SOC_min then 1
        elseif SOC_1 >= SOC_max then 0
        else pre(batNeedsChargingR);
      end when;
    equation
    ...
    //---------------- Greedy pass across ALL generators ( in the model) ----------------
      for i in 1:N loop
        if gen_exist[i] then
          (P_gen_cmd[i], P_bat_each[i], P_unmet_each[i], P_charge_each[i], P_node_collection[i]) = 
          calculateGeneratorPower(
                current_engine_indx = i, last_active_engine_indx = lastActive, 
                demand = P_rem[i], P_idle = P_idle[i], P_lo = P_opt_lo[i], 
                P_eff =  P_most_eff[i],    P_hi = P_opt_hi[i], 
                P_max = P_rated[i], useBattery = batteryInstalled, 
                Battery_Capability = P_cap_now, chargeIntent = batNeedsChargingR);
        else
          P_gen_cmd[i] = 0;
          P_bat_each[i] = 0;
...
// even if this is not the last generator, if there is no battery, the generaor will work at P_max
            if useBattery then
             yCand := pHi;   // non-last keeps pHi and passes the rest
            else 
             yCand := pMax;
           end if;
        end if;
    ...
    // SoC hysteresis for "needs charging"
      when initial() or SOC_1 <= SOC_min or SOC_1 >= SOC_max then
        batNeedsCharging := 
                      if SOC_1 <= SOC_min then 
                            true 
                      elseif SOC_1 >= SOC_max then 
                            false 
                      else pre(batNeedsCharging);
      end when;
    equation
    //---------------- Map optimal bounds to arrays ----------------
      P_opt_lo = {P_gen_1_Optim_Lower, P_gen_2_Optim_Lower, P_gen_3_Optim_Lower, P_gen_4_Optim_Lower, P_gen_5_Optim_Lower, P_gen_6_Optim_Lower, P_gen_7_Optim_Lower, P_gen_8_Optim_Lower, P_gen_9_Optim_Lower, P_gen_10_Optim_Lower};
    ...
    //---------------- Greedy pass across ALL generators ( in the model) ----------------
      for i in 1:N loop
        if gen_exist[i] then
          (P_gen_cmd[i], P_bat_each[i], P_unmet_each[i], P_charge_each[i]) = 
          calculateGeneratorPower(
                current_engine_indx = i, last_active_engine_indx = lastActive, 
                demand = P_rem[i], P_idle = P_idle[i], P_lo = P_opt_lo[i], 
                P_eff =  P_most_eff[i],    P_hi = P_opt_hi[i], 
                P_max = P_rated[i], useBattery = batteryInstalled, 
                Battery_Capability = P_cap_now, batteryNeedsCharging = batNeedsCharging);
        else
          P_gen_cmd[i] = 0;
          P_bat_each[i] = 0;
          P_unmet_each[i] = P_rem[i];
...
//---------------- Greedy pass across ALL generators ( in the model) ----------------
      for i in 1:N loop
        if gen_exist[i] then
          (P_gen_cmd[i], P_bat_each[i], P_unmet_each[i], P_charge_each[i]) = 
          calculateGeneratorPower(current_engine_indx = i, last_active_engine_indx = lastActive, demand = P_rem[i], P_idle = P_idle[i], P_lo = P_opt_lo[i], P_hi = P_opt_hi[i], P_max = P_rated[i], useBattery = batteryInstalled, Battery_Capability = P_cap_now, batteryNeedsCharging = batNeedsCharging);
        else
          P_gen_cmd[i] = 0;
          P_bat_each[i] = 0;
          P_unmet_each[i] = P_rem[i];
...
// before going into the loop, set the unmet to P_net for the first generartor
  ...
  /*
                    ...
                    */
    Modelica.Blocks.Sources.RealExpression PowertrainChargingPwr(y = individualControllerCore3.P_batt_charge_request) annotation(
    ...
    Modelica.Blocks.Math.Gain gainCharingMin(k = -1/(P_charging_min)) annotation(
      Placement(transformation(origin = {20, -6}, extent = {{-4, -4}, {4, 4}})));
  ...
  IndividualEngineController.IndividualControllerCore3 individualControllerCore3(
                            gen_1_exist = controller_gen_1_exist, gen_2_exist = controller_gen_2_exist, 
                            gen_3_exist = controller_gen_3_exist, gen_4_exist = controller_gen_4_exist, 
                            gen_5_exist = controller_gen_5_exist, gen_6_exist = controller_gen_6_exist, 
                            gen_7_exist = controller_gen_7_exist, gen_8_exist = controller_gen_8_exist, 
      ...
      Placement(transformation(origin = {-35.1538, 40.9872}, extent = {{-36.8462, -39.9167}, {36.8462, 39.9167}})));
  equation
    ...
    connect(gainCharingMin.y, P_discharge_1) annotation(
      Line(points = {{24, -6}, {50, -6}, {50, -110}}, color = {0, 85, 0}, pattern = LinePattern.DashDotDot, thickness = 1));
    connect(gainCharingMax.y, P_charge_1) annotation(
      Line(points = {{14, -30}, {32, -30}, {32, -110}, {30, -110}}, color = {0, 0, 127}, thickness = 1));
  connect(gen_1_controller_optim_upper, individualControllerCore3.P_gen_1_Optim_Upper) annotation(
      Line(points = {{-110, 90}, {-88, 90}, {-88, 69}, {-69, 69}}, color = {0, 0, 127}));
  connect(gen_1_controller_optim_lower, individualControllerCore3.P_gen_1_Optim_Lower) annotation(
      Line(points = {{-110, 70}, {-89.5, 70}, {-89.5, 62}, {-69, 62}}, color = {0, 0, 127}));
  connect(gen_2_controller_optim_upper, individualControllerCore3.P_gen_2_Optim_Upper) annotation(
      Line(points = {{-110, 50}, {-92, 50}, {-92, 56}, {-69, 56}}, color = {0, 0, 127}));
  connect(gen_2_controller_optim_lower, individualControllerCore3.P_gen_2_Optim_Lower) annotation(
      Line(points = {{-110, 30}, {-90, 30}, {-90, 50}, {-69, 50}}, color = {0, 0, 127}));
  connect(gen_3_controller_optim_upper, individualControllerCore3.P_gen_3_Optim_Upper) annotation(
      Line(points = {{-110, 10}, {-86, 10}, {-86, 44}, {-69, 44}}, color = {0, 0, 127}));
  connect(gen_3_controller_optim_lower, individualControllerCore3.P_gen_3_Optim_Lower) annotation(
      Line(points = {{-110, -10}, {-82, -10}, {-82, 38}, {-69, 38}}, color = {0, 0, 127}));
  connect(gen_4_controller_optim_upper, individualControllerCore3.P_gen_4_Optim_Upper) annotation(
      Line(points = {{-110, -30}, {-80, -30}, {-80, 32}, {-69, 32}}, color = {0, 0, 127}));
  connect(gen_4_controller_optim_lower, individualControllerCore3.P_gen_4_Optim_Lower) annotation(
      Line(points = {{-110, -50}, {-90, -50}, {-90, 26}, {-69, 26}}, color = {0, 0, 127}));
  connect(gen_5_controller_optim_upper, individualControllerCore3.P_gen_5_Optim_Upper) annotation(
      Line(points = {{-110, -70}, {-84, -70}, {-84, 19}, {-69, 19}}, color = {0, 0, 127}));
  connect(gen_5_controller_optim_lower, individualControllerCore3.P_gen_5_Optim_Lower) annotation(
      Line(points = {{-110, -90}, {-88, -90}, {-88, 13}, {-69, 13}}, color = {0, 0, 127}));
  connect(P_load_1, individualControllerCore3.P_load_1) annotation(
      Line(points = {{-66, -114}, {-66, -52}, {-62, -52}, {-62, 7}}, color = {0, 0, 127}));
  connect(SOC_1, individualControllerCore3.SOC_1) annotation(
      Line(points = {{-92, -120}, {-84, -120}, {-84, 7}, {-56, 7}}, color = {0, 0, 127}));
  connect(genMinBSFC1, individualControllerCore3.genMostEffPwr1) annotation(
      Line(points = {{-50, -106}, {-50, 6}, {-38, 6}, {-38, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC2, individualControllerCore3.genMostEffPwr2) annotation(
      Line(points = {{-42, -106}, {-42, 4}, {-35, 4}, {-35, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC3, individualControllerCore3.genMostEffPwr3) annotation(
      Line(points = {{-34, -106}, {-34, -46}, {-32, -46}, {-32, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC4, individualControllerCore3.genMostEffPwr4) annotation(
      Line(points = {{-26, -106}, {-26, 0}, {-29, 0}, {-29, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC5, individualControllerCore3.genMostEffPwr5) annotation(
      Line(points = {{-18, -106}, {-22, -106}, {-22, 8}, {-26, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC6, individualControllerCore3.genMostEffPwr6) annotation(
      Line(points = {{-10, -106}, {-12, -106}, {-12, 8}, {-23, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC7, individualControllerCore3.genMostEffPwr7) annotation(
      Line(points = {{-2, -106}, {-2, -44}, {-18, -44}, {-18, 8}, {-20, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC8, individualControllerCore3.genMostEffPwr8) annotation(
      Line(points = {{6, -106}, {6, -82}, {-16, -82}, {-16, 8}, {-17, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC9, individualControllerCore3.genMostEffPwr9) annotation(
      Line(points = {{14, -106}, {14, -72}, {-14, -72}, {-14, 8}}, color = {0, 0, 127}));
  connect(genMinBSFC10, individualControllerCore3.genMostEffPwr10) annotation(
      Line(points = {{22, -106}, {22, -78}, {-8, -78}, {-8, 6}, {-11, 6}, {-11, 8}}, color = {0, 0, 127}));
  connect(individualControllerCore3.P_battery_1, gainCharingMin.u) annotation(
      Line(points = {{-1, 7}, {6, 7}, {6, -6}, {16, -6}}, color = {0, 0, 127}));
  connect(gen_6_controller_optim_upper, individualControllerCore3.P_gen_6_Optim_Upper) annotation(
      Line(points = {{-90, 110}, {-90, 88}, {-63, 88}, {-63, 75}}, color = {0, 0, 127}));
  connect(gen_6_controller_optim_lower, individualControllerCore3.P_gen_6_Optim_Lower) annotation(
      Line(points = {{-70, 110}, {-70, 86}, {-57, 86}, {-57, 75}}, color = {0, 0, 127}));
  connect(gen_7_controller_optim_upper, individualControllerCore3.P_gen_7_Optim_Upper) annotation(
      Line(points = {{-50, 110}, {-50, 90}, {-51, 90}, {-51, 75}}, color = {0, 0, 127}));
  connect(gen_7_controller_optim_lower, individualControllerCore3.P_gen_7_Optim_Lower) annotation(
      Line(points = {{-30, 110}, {-30, 90}, {-44, 90}, {-44, 75}}, color = {0, 0, 127}));
  connect(gen_8_controller_optim_upper, individualControllerCore3.P_gen_8_Optim_Upper) annotation(
      Line(points = {{-10, 110}, {-10, 88}, {-38, 88}, {-38, 75}}, color = {0, 0, 127}));
  connect(gen_8_controller_optim_lower, individualControllerCore3.P_gen_8_Optim_Lower) annotation(
      Line(points = {{10, 110}, {10, 86}, {-32, 86}, {-32, 75}}, color = {0, 0, 127}));
  connect(gen_9_controller_optim_upper, individualControllerCore3.P_gen_9_Optim_Upper) annotation(
      Line(points = {{30, 110}, {30, 92}, {-26, 92}, {-26, 75}}, color = {0, 0, 127}));
  connect(gen_9_controller_optim_lower, individualControllerCore3.P_gen_9_Optim_Lower) annotation(
      Line(points = {{50, 110}, {50, 90}, {-20, 90}, {-20, 75}}, color = {0, 0, 127}));
  connect(gen_10_controller_optim_upper, individualControllerCore3.P_gen_10_Optim_Upper) annotation(
      Line(points = {{70, 110}, {72, 110}, {72, 91}, {-14, 91}, {-14, 75}}, color = {0, 0, 127}));
  connect(gen_10_controller_optim_lower, individualControllerCore3.P_gen_10_Optim_Lower) annotation(
      Line(points = {{90, 110}, {90, 84}, {-8, 84}, {-8, 75}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen1_output, generator_1_controller_output) annotation(
      Line(points = {{-1, 69}, {66, 69}, {66, 90}, {110, 90}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen2_output, generator_2_controller_output) annotation(
      Line(points = {{-1, 62}, {54.5, 62}, {54.5, 70}, {110, 70}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen3_output, generator_3_controller_output) annotation(
      Line(points = {{-1, 56}, {60, 56}, {60, 50}, {110, 50}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen4_output, generator_4_controller_output) annotation(
      Line(points = {{-1, 50}, {26, 50}, {26, 30}, {110, 30}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen5_output, generator_5_controller_output) annotation(
      Line(points = {{-1, 44}, {54, 44}, {54, 10}, {110, 10}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen6_output, generator_6_controller_output) annotation(
      Line(points = {{-1, 38}, {66, 38}, {66, -10}, {110, -10}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen7_output, generator_7_controller_output) annotation(
      Line(points = {{-1, 32}, {72, 32}, {72, -32}, {110, -32}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen8_output, generator_8_controller_output) annotation(
      Line(points = {{-1, 26}, {24, 26}, {24, 18}, {78, 18}, {78, -50}, {110, -50}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen9_output, generator_9_controller_output) annotation(
      Line(points = {{-1, 19}, {42, 19}, {42, 6}, {62, 6}, {62, -70}, {110, -70}}, color = {0, 0, 127}));
  connect(individualControllerCore3.gen10_output, generator_10_controller_output) annotation(
      Line(points = {{-1, 13}, {20, 13}, {20, 10}, {50, 10}, {50, 0}, {58, 0}, {58, -90}, {110, -90}}, color = {0, 0, 127}));
    annotation(
      Diagram,
  ...
  model IdealTankSinglePort
    ...
    model TriplePortTank
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.Hydrogen constrainedby AZEAT_Indi_Engine_Control_Test_3.Fuel.SimpleFuel "Transported medium";
      ...
      /*Fuel*/
      //Fuel model will be referenced From AZEAT_Indi_Engine_Control_Test_3
      replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.Hydrogen constrainedby AZEAT_Indi_Engine_Control_Test_3.Fuel.Templates.SimpleFuel "Transported medium" annotation(
         choicesAllMatching);
...
// Find x values where y crosses 110% of min (first and last crossing)
  ...
  model OptimalFuelConsumptionBoundsCalculator_ExternalTable
    function argmin
      "Return index of first minimum element of a vector"
      input Real v[:] ;
      output Integer iMin;
    protected
      Integer i;
...
// Find how many units are required to run
  ...
  package Fuel
    ...
    package compressedhydrogen
      extends AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel(LHV = 120e6, rho = 26, rho_liq = 26, MolarMass = 1.00899e-3, Delta_h_evap = 2.01588*1e-3);
  ...
  model Generator
    ...
    model Generator_Base
      ...
      replaceable package Fuel = AZEAT_Indi_Engine_Control_Test_3.Fuel.Diesel constrainedby AZEAT_Indi_Engine_Control_Test_3.Fuel.SimpleFuel "Fuel type" annotation(
         choicesAllMatching,
      ...
      FluidPort co2_outlet(redeclare package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.CarbonDioxide, m_flow = -m_flow_CO2) if use_external_co2_sink annotation(
        Placement(transformation(extent = {{-10.0, -110.0}, {10.0, -90.0}}, rotation = 0.0, origin = {0.0, 0.0})));
    protected
...
// for mathematical stability , if constant efficiency is used this variable is not used in computation but it should be assigned with something for numerical balance
  ...
  model Tank "Tank model for gas storage"
    ...
    partial model Tank_base
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.Hydrogen constrainedby AZEAT_Indi_Engine_Control_Test_3Fuel.Templates.SimpleFuel "Transported medium";
        Modelica.Units.SI.Pressure p "Pressure";
        flow Modelica.Units.SI.MassFlowRate m_flow "Mass flow rate";
        annotation(
          Icon(coordinateSystem(preserveAspectRatio = false), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {28, 108, 200}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid)}),
      ...
      replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.compressedhydrogen "Transported Medium" annotation(
        choicesAllMatching);
    ...
    //EndRepositroy
    extends Tank_base(E_store = energyDensity_max*V, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.compressedhydrogen constrainedby AZEAT_Indi_Engine_Control_Test_3.Fuel.Templates.SimpleFuel);
    //Local Parameters
...
// Connectors
  ...
  model FuelCellEfficiency
    partial model Base
      ...
      replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.Hydrogen constrainedby AZEAT_Indi_Engine_Control_Test_3.Fuel.Hydrogen "Transported medium" annotation(
         choicesAllMatching);
...
//fuel parameter Generator_Fuel GEN 3
  ...
  /*ENGINE PARAMETER*/
  parameter Real generator_P_rat_1 = 808000;
  parameter Real generator_P_rat_2 = 808000;
  ...
  parameter Real mCtrl_user_defined_lower_bound_diesel_1 = 300;
  ...
  parameter Real mCtrl_user_defined_lower_bound_diesel_2 = 300;
  ...
  parameter Real battery_Eff_Charge = 0.95;
  parameter Boolean battery_Set_SOC_Final_Start = true;
  parameter Real battery_Capacity = 2.239e+8  ;
  parameter Real battery_P_max = 93240; //93240
  ...
  parameter Real BSFC_Curve_1[:, 2] = [80, 197.91; 160, 189.97; 240, 183.23; 320, 177.71; 400, 173.40; 480, 170.30; 560, 168.42; 640, 167.74; 720, 168.28; 808, 170.03];
  parameter Real BSFC_Curve_2[:, 2] = [80, 197.91; 160, 189.97; 240, 183.23; 320, 177.71; 400, 173.40; 480, 170.30; 560, 168.42; 640, 167.74; 720, 168.28; 808, 170.03];
  ...
  // Part Battery
  AZEAT_Indi_Engine_Control_Test_3.BatteryDualControl battery1(eff_charge = battery_Eff_Charge, set_SOC_final_start_ = battery_Set_SOC_Final_Start, SOC_max = mCtrl_SOC_max, SOC_min = mCtrl_SOC_min, capacity = battery_Capacity, P_max = battery_P_max, SOC_start = battery_SOC_start) annotation(
    Placement(transformation(origin = {-60, -138}, extent = {{-18, -18}, {18, 18}})));
  // Part Electrical Untitlity
  AZEAT_Indi_Engine_Control_Test_3.Converter_ACDC converter_ACDC(efficiency = 1, V_ref_DC = 1, power_nominal = P_nominal, P_max = 10*P_nominal) "Inverter" annotation(
    Placement(transformation(origin = {144, -222}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.ElectricalGrid electricalGrid(power_nominal = P_nominal, V_ref = 1000) annotation(
    Placement(transformation(origin = {192, -224}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.ElectricLoad electricLoad(use_input = true, load_in = 1000) annotation(
    Placement(transformation(origin = {167, -201}, extent = {{11, -11}, {-11, 11}})));
  AZEAT_Indi_Engine_Control_Test_3.Transformer transformer(efficiency = 1, power_nominal = P_nominal, P_max = 10*P_nominal) annotation(
  ...
  // Part Controller
  Modelica.Blocks.Tables.CombiTable1Ds combiTable1Ds(tableOnFile = true, tableName = "tab1", fileName = "C:/Users/NMS08/Desktop/ReCreatedModel/AZETValidationCase/2024-04_FC_Cardinal_buoy_maintenance Adjusted.txt", verboseRead = true, columns = 2:2) annotation(
    Placement(transformation(origin = {228, -274}, extent = {{10, -10}, {-10, 10}})));
  ...
  AZEAT_Indi_Engine_Control_Test_3.IndividualEngineController individualEngineController(
                  controller_gen_1_exist = gen1_is_on, controller_gen_2_exist = gen2_is_on, 
                  controller_gen_3_exist = gen3_is_on, controller_gen_4_exist = gen4_is_on, 
                  controller_gen_5_exist = gen5_is_on, controller_gen_6_exist = gen6_is_on, 
                  controller_gen_7_exist = gen7_is_on, controller_gen_8_exist = gen8_is_on, 
                  controller_gen_9_exist = gen9_is_on, controller_gen_10_exist = gen10_is_on, 
                  controller_gen_1_idle  = generator_P_idle_1,  controller_gen_2_idle  = generator_P_idle_2,
                  controller_gen_3_idle  = generator_P_idle_3,  controller_gen_4_idle  = generator_P_idle_4,
                  controller_gen_5_idle  = generator_P_idle_5,  controller_gen_6_idle  = generator_P_idle_6,
                  controller_gen_7_idle  = generator_P_idle_7,  controller_gen_8_idle  = generator_P_idle_8,
                  controller_gen_9_idle  = generator_P_idle_9,  controller_gen_10_idle  = generator_P_idle_10,
  ...
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen1(BSFC_Curve_Default = BSFC_Curve_1, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_1, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_1, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_1,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_1, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_1) annotation(
    Placement(transformation(origin = {-514, 270}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen2(BSFC_Curve_Default = BSFC_Curve_2, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_2, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_2, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_2,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_2, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_2) annotation(
    Placement(transformation(origin = {-512, 194}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen3(BSFC_Curve_Default = BSFC_Curve_3, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_3, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_3, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_3,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_3, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_3 ) annotation(
    Placement(transformation(origin = {-510, 124}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen4(BSFC_Curve_Default = BSFC_Curve_4, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_4, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_4, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_4,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_4, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_4) annotation(
    Placement(transformation(origin = {-510, 74}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen5(BSFC_Curve_Default = BSFC_Curve_5, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_5, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_5, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_5,
  ...
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen6(BSFC_Curve_Default = BSFC_Curve_6, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_6, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_6, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_6,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_6, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_6 ) annotation(
    Placement(transformation(origin = {-330, 280}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen7(BSFC_Curve_Default = BSFC_Curve_7, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_7, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_7, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_7,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_7, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_7) annotation(
    Placement(transformation(origin = {-328, 214}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen8(BSFC_Curve_Default = BSFC_Curve_8, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_8, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_8, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_8,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_8, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_8) annotation(
    Placement(transformation(origin = {-330, 150}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen9(BSFC_Curve_Default = BSFC_Curve_9, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_9, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_9, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_9,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_9, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_9) annotation(
    Placement(transformation(origin = {-186, 290}, extent = {{10, 10}, {-10, -10}})));
  OptimalFuelConsumptionBoundsCalculator_ExternalTable bsfc_table_gen10(BSFC_Curve_Default = BSFC_Curve_10, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel_10, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel_10, manually_define_bounds = mCtrl_user_defined_Bounds_diesel_10,
  minimal_bsfc_percentage = mCtrl_user_defined_BSFC_percentage_10, manual_defined_most_effcient_power = mCtrl_user_defined_most_eff_ouput_10) annotation(
  ...
  AZEAT_Indi_Engine_Control_Test_3.Generator generator1(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_1, 
      V_flow_frat = generator_V_flow_Frat_1 , 
      V_flow_fidle = generator_V_flow_Fidle_1, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_1, 
      m_flow_frat = generator_M_Flow_FRat_1, 
      P_rat = generator_P_rat_1, 
      FLHV = generator_FLHV_1, 
      Frho = generator_Frho_1, 
      Frho_liq = generator_Frho_liq_1, 
      FcarbonContent = generator_FcarbonContent_1, 
      FMolarMass = generator_MolarMass_1, use_fuel_eff_table = generator_use_fuel_eff_table_1
  ) annotation(
    Placement(transformation(origin = {8, 290}, extent = {{10, -10}, {-10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator2(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_2, 
      V_flow_frat = generator_V_flow_Frat_2, 
      V_flow_fidle = generator_V_flow_Fidle_2, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_2, 
      m_flow_frat = generator_M_Flow_FRat_2, 
      P_rat = generator_P_rat_2, 
      FLHV = generator_FLHV_2, 
      Frho = generator_Frho_2, 
  ...
  AZEAT_Indi_Engine_Control_Test_3.Generator generator3(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_3, 
      V_flow_frat = generator_V_flow_Frat_3 , 
      V_flow_fidle = generator_V_flow_Fidle_3, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_3, 
      m_flow_frat = generator_M_Flow_FRat_3, 
      P_rat = generator_P_rat_3, 
      FLHV = generator_FLHV_3, 
      Frho = generator_Frho_3, 
      Frho_liq = generator_Frho_liq_3, 
      FcarbonContent = generator_FcarbonContent_3, 
      FMolarMass = generator_MolarMass_3, use_fuel_eff_table = generator_use_fuel_eff_table_3
  ) annotation(
    Placement(transformation(origin = {10, 210}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator4(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_4, 
      V_flow_frat = generator_V_flow_Frat_4, 
      V_flow_fidle = generator_V_flow_Fidle_4, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_4, 
      m_flow_frat = generator_M_Flow_FRat_4, 
      P_rat = generator_P_rat_4, 
      FLHV = generator_FLHV_4, 
      Frho = generator_Frho_4, 
      Frho_liq = generator_Frho_liq_4, 
      FcarbonContent = generator_FcarbonContent_4, 
      FMolarMass = generator_MolarMass_4, use_fuel_eff_table = generator_use_fuel_eff_table_4
  ) annotation(
    Placement(transformation(origin = {10, 170}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator5(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_5, 
      V_flow_frat = generator_V_flow_Frat_5 , 
      V_flow_fidle = generator_V_flow_Fidle_5, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_5, 
      m_flow_frat = generator_M_Flow_FRat_5, 
      P_rat = generator_P_rat_5, 
      FLHV = generator_FLHV_5, 
      Frho = generator_Frho_5, 
      Frho_liq = generator_Frho_liq_5, 
      FcarbonContent = generator_FcarbonContent_5, 
      FMolarMass = generator_MolarMass_5, use_fuel_eff_table = generator_use_fuel_eff_table_5
  ) annotation(
    Placement(transformation(origin = {10, 130}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator6(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_6, 
      V_flow_frat = generator_V_flow_Frat_6 , 
      V_flow_fidle = generator_V_flow_Fidle_6, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_6, 
      m_flow_frat = generator_M_Flow_FRat_6, 
      P_rat = generator_P_rat_6, 
      FLHV = generator_FLHV_6, 
      Frho = generator_Frho_6, 
      Frho_liq = generator_Frho_liq_6, 
      FcarbonContent = generator_FcarbonContent_6, 
      FMolarMass = generator_MolarMass_6, use_fuel_eff_table = generator_use_fuel_eff_table_6
  ) annotation(
    Placement(transformation(origin = {52, 50}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator7(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_7, 
      V_flow_frat = generator_V_flow_Frat_7,
      V_flow_fidle = generator_V_flow_Fidle_7, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_7, 
      m_flow_frat = generator_M_Flow_FRat_7, 
      P_rat = generator_P_rat_7, 
      FLHV = generator_FLHV_7, 
      Frho = generator_Frho_7, 
      Frho_liq = generator_Frho_liq_7, 
      FcarbonContent = generator_FcarbonContent_7, 
      FMolarMass = generator_MolarMass_7, use_fuel_eff_table = generator_use_fuel_eff_table_7
  ) annotation(
    Placement(transformation(origin = {52, 10}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator8(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_8, 
      V_flow_frat = generator_V_flow_Frat_8, 
      V_flow_fidle = generator_V_flow_Fidle_8, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_8, 
      m_flow_frat = generator_M_Flow_FRat_8, 
      P_rat = generator_P_rat_8, 
      FLHV = generator_FLHV_8, 
      Frho = generator_Frho_8, 
      Frho_liq = generator_Frho_liq_8, 
      FcarbonContent = generator_FcarbonContent_8, 
      FMolarMass = generator_MolarMass_8, use_fuel_eff_table = generator_use_fuel_eff_table_8
  ) annotation(
    Placement(transformation(origin = {52, -30}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator9(
      power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_9, 
      V_flow_frat = generator_V_flow_Frat_9, 
      V_flow_fidle = generator_V_flow_Fidle_9, 
      use_const_eff = generator_Use_Const_Off, 
      eta_el_const = generator_Eta_El_Const, 
      fuelConsumptionIsVolumetric = true, 
      m_flow_fidle = generator_M_Flow_Fidle_9, 
      m_flow_frat = generator_M_Flow_FRat_9, 
      P_rat = generator_P_rat_9, 
      FLHV = generator_FLHV_9, 
      Frho = generator_Frho_9, 
      Frho_liq = generator_Frho_liq_9, 
      FcarbonContent = generator_FcarbonContent_9, 
      FMolarMass = generator_MolarMass_9, use_fuel_eff_table = generator_use_fuel_eff_table_9
  ) annotation(
    Placement(transformation(origin = {52, -70}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.Generator generator10(
    power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle_10, 
    V_flow_frat = generator_V_flow_Frat_10 , 
    V_flow_fidle = generator_V_flow_Fidle_10, 
    use_const_eff = generator_Use_Const_Off, 
    eta_el_const = generator_Eta_El_Const, 
    fuelConsumptionIsVolumetric = true, 
    m_flow_fidle = generator_M_Flow_Fidle_10, 
    m_flow_frat = generator_M_Flow_FRat_10, 
    P_rat = generator_P_rat_10, 
    FLHV = generator_FLHV_10, 
    Frho = generator_Frho_10, 
    Frho_liq = generator_Frho_liq_10, 
    FcarbonContent = generator_FcarbonContent_10, 
    FMolarMass = generator_MolarMass_10, use_fuel_eff_table = generator_use_fuel_eff_table_10
  ) annotation(
    Placement(transformation(origin = {52, -110}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort1(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {90, 270}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort2(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {90, 230}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort3(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {90, 190}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort4(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {90, 150}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort5(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {90, 110}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort6(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel)  annotation(
    Placement(transformation(origin = {110, 30}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort7(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel)  annotation(
    Placement(transformation(origin = {110, -10}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort8(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel)  annotation(
    Placement(transformation(origin = {110, -50}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort9(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel)  annotation(
    Placement(transformation(origin = {110, -90}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Indi_Engine_Control_Test_3.IdealTankSinglePort idealTankSinglePort10(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Indi_Engine_Control_Test_3.Fuel.MethanolFuel)  annotation(
  ...
  Modelica.Blocks.Sources.RealExpression genFuelFlow1(y = generator1.m_flow_fuel )  annotation(
    Placement(transformation(origin = {-490, -40}, extent = {{-10, -10}, {10, 10}})));
  ...
  Modelica.Blocks.Sources.RealExpression genFuelFlow10(y = if gen10_is_on then generator10.outlet_fuel.m_flow else 0) annotation(
    Placement(transformation(origin = {-484, -382}, extent = {{-10, -10}, {10, 10}})));
   Modelica.Blocks.Sources.RealExpression Surplus(y= (((if gen1_is_on then generator1.P_out else 0) + (if gen2_is_on then generator2.P_out else 0) + (if gen3_is_on then generator3.P_out else 0) +
      (if gen4_is_on then generator4.P_out else 0) + (if gen5_is_on then generator5.P_out else 0) + (if gen6_is_on then generator6.P_out else 0) + (if gen7_is_on then generator7.P_out else 0)  +
      (if gen8_is_on then generator8.P_out else 0) + (if gen9_is_on then generator9.P_out else 0) + (if gen10_is_on then generator10.P_out else 0)   
      )+(if battery_P_max> 1 then battery1.P_discharge_abs else 0 )-(gain1.y))) annotation(
    Placement(transformation(origin = {-350, -90}, extent = {{-10, -10}, {10, 10}})));

  Modelica.Blocks.Interaction.Show.RealValue reavalueSurplus annotation(
    Placement(transformation(origin = {-270, -90}, extent = {{-10, -10}, {10, 10}})));
  ...
  Modelica.Blocks.Continuous.Integrator integrator1(initType = Modelica.Blocks.Types.Init.InitialState, y_start = 0, k = 1)  annotation(
    Placement(transformation(origin = {-450, -40}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator3 annotation(
    Placement(transformation(origin = {-444, -234}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator4 annotation(
    Placement(transformation(origin = {-444, -262}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator5 annotation(
    Placement(transformation(origin = {-444, -282}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator6 annotation(
    Placement(transformation(origin = {-444, -302}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator7 annotation(
    Placement(transformation(origin = {-444, -322}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator8 annotation(
    Placement(transformation(origin = {-444, -342}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator9 annotation(
    Placement(transformation(origin = {-444, -362}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.Integrator integrator10 annotation(
    Placement(transformation(origin = {-444, -382}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interaction.Show.RealValue realValueGenFuelConsump1 annotation(
    Placement(transformation(origin = {-408, -40}, extent = {{-10, -10}, {10, 10}})));
 ...
 AZEAT_Indi_Engine_Control_Test_3.FuelUsageTrapezoidSampled fuelUsageTrapezoidSampled annotation(
    Placement(transformation(origin = {-450, -74}, extent = {{-10, -10}, {10, 10}})));
 Modelica.Blocks.Interaction.Show.RealValue realFuelUsage1 annotation(
    Placement(transformation(origin = {-410, -70}, extent = {{-10, -10}, {10, 10}}))); initial equation
    fuelUsage1 = 0;
    fuelUsage2 = 0;
    fuelUsage3 = 0;
    fuelUsage4 = 0;
 ...
 equation
  ...
  connect(time_expre2.y, combiTable1Ds.u) annotation(
    Line(points = {{295, -276}, {267, -276}, {267, -274}, {240, -274}}, color = {0, 0, 127}));
  connect(combiTable1Ds.y[1], gain1.u) annotation(
    Line(points = {{217, -274}, {171, -274}, {171, -276}}, color = {0, 0, 127}));
  ...
  connect(integrator11.y, realValueTotalFuelCosumption.numberPort) annotation(
    Line(points = {{-298, -50}, {-282, -50}}, color = {0, 0, 127}));
  ...
  connect(Surplus.y, reavalueSurplus.numberPort) annotation(
    Line(points = {{-338, -90}, {-282, -90}}, color = {0, 0, 127}));
 connect(genFuelFlow1.y, fuelUsageTrapezoidSampled.u) annotation(
    Line(points = {{-478, -40}, {-474, -40}, {-474, -73}, {-461, -73}}, color = {0, 0, 127}));
 connect(fuelUsageTrapezoidSampled.y, realFuelUsage1.numberPort) annotation(
    Line(points = {{-439, -74}, {-429.5, -74}, {-429.5, -70}, {-422, -70}}, color = {0, 0, 127}));
  annotation(
    ...
    uses(Modelica(version = "4.0.0")),
    Diagram(coordinateSystem(extent = {{-520, 320}, {320, -400}})),
    version = "",
    __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=NLSanalyticJacobian",
    __OpenModelica_simulationFlags(lv = "LOG_STDOUT,LOG_ASSERT,LOG_STATS", s = "dassl", variableFilter = ".*"));
end AZEAT_Indi_Engine_Control_Test_3;
