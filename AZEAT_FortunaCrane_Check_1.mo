model AZEAT_FortunaCrane_Check_1
  //=============
  ...
  // External Model Repository for Local AZEAT_FortunaCrane_Check_1 Use
  ...
  model MasterControllerSingleBattery
    ...
    // When demand is  neither  larger than rated power not within higher bound and  rated power
          ...
          elseif iceChargingProrityAssignement == 1 then
            ...
            elseif (P_net < P_idling_diesel_gen or P_net == P_idling_diesel_gen) then
              ...
              if (chargingStatus) then
                P_dieselGen = 0 ; // if charging is required,  dieselgen can not be bothered torunm and methanol engine is used to charge the battery.
                P_altFuelGen =  if (P_rated_battery > 1) then AlthFuelGenChargingRate else 0;
                P_battery_1 = 0; 
                P_fuelCellGen = 0;
                IceGenChargingRate = if (P_rated_battery > 1) then AlthFuelGenChargingRate else 0;  // if charging is requrid andthere is a battery then, charge it wil excess power
              else
    ...
    // Prioritise Alt Fuel, Diesel Charging , Demand within the BSFC Sweet Spot Zone
            ...
            elseif (P_net < P_idling_alt_gen or P_net == P_idling_alt_gen) then
              ...
              if (chargingStatus) then
                P_dieselGen = 0 ;
                P_altFuelGen =  if (P_rated_battery > 1) then AltFuelGeneratorCount*altFuelOptimLowerBound  else P_idling_alt_gen; // if charging is required,  methanol generator runs the lowr bound, excess ppower is delieved into battery .
                P_battery_1 = 0; 
                P_fuelCellGen = 0;
                IceGenChargingRate = if (P_rated_battery > 1) then AltFuelGeneratorCount*altFuelOptimLowerBound - P_net else 0;  // if charging is requrid andthere is a battery then, charge it wil excess power
              else
                // When Charging is not required
                P_dieselGen = 0;
                P_altFuelGen = 0;
          ...
          elseif iceChargingProrityAssignement == 1 then
            ...
            elseif (P_net < P_idling_alt_gen or P_net == P_idling_alt_gen) then
              ...
              if (chargingStatus) then
                ...
                P_altFuelGen =   0; // if charging is required then the thing
                P_battery_1 = 0; 
                P_fuelCellGen = 0;
                IceGenChargingRate = if (P_rated_battery > 1) then DieselGenChargingRate else 0;  // if charging is requrid andthere is a battery then, charge it wil excess power
              else
                P_dieselGen = 0; // When Charging is not required
                P_altFuelGen = 0;
                P_battery_1 = P_net; 
                P_fuelCellGen = 0;
                IceGenChargingRate = 0 ;
...
// Find how many units are required to run
  ...
  package Fuel
    ...
    package compressedhydrogen
      extends AZEAT_FortunaCrane_Check_1.Fuel.MethanolFuel(LHV = 120e6, rho = 26, rho_liq = 26, MolarMass = 1.00899e-3, Delta_h_evap = 2.01588*1e-3);
  ...
  model Generator
    ...
    model Generator_Base
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.Hydrogen constrainedby AZEAT_FortunaCrane_Check_1.Fuel.SimpleFuel "Transported medium";
      ...
      replaceable package Fuel = AZEAT_FortunaCrane_Check_1.Fuel.Diesel constrainedby AZEAT_FortunaCrane_Check_1.Fuel.SimpleFuel "Fuel type" annotation(
         choicesAllMatching,
      ...
      FluidPort co2_outlet(redeclare package Medium = AZEAT_FortunaCrane_Check_1.Fuel.CarbonDioxide, m_flow = -m_flow_CO2) if use_external_co2_sink annotation(
        Placement(transformation(extent = {{-10.0, -110.0}, {10.0, -90.0}}, rotation = 0.0, origin = {0.0, 0.0})));
    protected
...
// for mathematical stability , if constant efficiency is used this variable is not used in computation but it should be assigned with something for numerical balance
  ...
  model IdealTankMultiPort
    ...
    model TriplePortTank
      ...
      /*Fuel*/
      //Fuel model will be referenced From AZEAT_FortunaCrane_Check_1
      replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.Hydrogen constrainedby AZEAT_FortunaCrane_Check_1.Fuel.Templates.SimpleFuel "Transported medium" annotation(
         choicesAllMatching);
  ...
  model Tank "Tank model for gas storage"
    ...
    partial model Tank_base
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.Hydrogen constrainedby AZEAT_FortunaCrane_Check_1Fuel.Templates.SimpleFuel "Transported medium";
        Modelica.Units.SI.Pressure p "Pressure";
        flow Modelica.Units.SI.MassFlowRate m_flow "Mass flow rate";
        annotation(
          Icon(coordinateSystem(preserveAspectRatio = false), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {28, 108, 200}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid)}),
      ...
      replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.compressedhydrogen "Transported Medium" annotation(
        choicesAllMatching);
    ...
    //EndRepositroy
    extends Tank_base(E_store = energyDensity_max*V, redeclare replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.compressedhydrogen constrainedby AZEAT_FortunaCrane_Check_1.Fuel.Templates.SimpleFuel);
    //Local Parameters
...
// Connectors
  ...
  model FuelCellEfficiency
    partial model Base
      ...
      replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.Hydrogen constrainedby AZEAT_FortunaCrane_Check_1.Fuel.Hydrogen "Transported medium" annotation(
         choicesAllMatching);
...
//[0, 0.00; 10, 16.74; 20, 31.87; 30, 46.45; 40, 60.68; 50, 74.66; 60, 88.44; 70, 102.06; 80, 115.54; 90, 128.90; 100, 142.16] old
  ...
  // Part Battery
  AZEAT_FortunaCrane_Check_1.BatteryDualControl battery1(
  ...
  AZEAT_FortunaCrane_Check_1.Converter_ACDC converter_ACDC(efficiency = 1, V_ref_DC = 1, power_nominal = P_nominal, P_max = 10*P_nominal) "Inverter" annotation(
    Placement(transformation(origin = {108, 54}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_FortunaCrane_Check_1.ElectricalGrid electricalGrid(power_nominal = P_nominal, V_ref = 1000) annotation(
    Placement(transformation(origin = {156, 52}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_FortunaCrane_Check_1.ElectricLoad electricLoad(use_input = true, load_in = 1000) annotation(
    Placement(transformation(origin = {71, 29}, extent = {{11, -11}, {-11, 11}})));
  AZEAT_FortunaCrane_Check_1.Transformer transformer(efficiency = 1, power_nominal = P_nominal, P_max = 10*P_nominal) annotation(
  ...
  // Part Mechanical
  AZEAT_FortunaCrane_Check_1.Generator generator1(
  ...
  AZEAT_FortunaCrane_Check_1.Generator generator2(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle, V_flow_frat = generator_V_flow_Frat, V_flow_fidle = generator_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_M_Flow_Fidle, m_flow_frat = generator_M_Flow_FRat, P_rat = generator_P_rat, FLHV = generator_FLHV, Frho = generator_Frho, Frho_liq = generator_Frho_liq, FcarbonContent = generator_FcarbonContent, FMolarMass = generator_MolarMass, use_fuel_eff_table = generator_use_fuel_eff_table) annotation(
    Placement(transformation(origin = {-10, -25}, extent = {{10, -9}, {-10, 9}})));
  AZEAT_FortunaCrane_Check_1.Generator generator3(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle, V_flow_frat = generator_V_flow_Frat, V_flow_fidle = generator_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_M_Flow_Fidle, m_flow_frat = generator_M_Flow_FRat, P_rat = generator_P_rat, FLHV = generator_FLHV, Frho = generator_Frho, Frho_liq = generator_Frho_liq, FcarbonContent = generator_FcarbonContent, FMolarMass = generator_MolarMass, use_fuel_eff_table = generator_use_fuel_eff_table) annotation(
    Placement(transformation(origin = {-10, -56}, extent = {{10, -10}, {-10, 10}})));
  AZEAT_FortunaCrane_Check_1.Generator generator_af_1(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, use_fuel_eff_table = generator_alt_fuel_eff_table, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -90}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_FortunaCrane_Check_1.Generator generator_af_2(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, use_fuel_eff_table = generator_alt_fuel_eff_table, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -130}, extent = {{10, -10}, {-10, 10}})));
  AZEAT_FortunaCrane_Check_1.Generator generator_af_3(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, use_fuel_eff_table = generator_alt_fuel_eff_table, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -170}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_FortunaCrane_Check_1.IdealTankMultiPort idealTankMultiPort(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {70, -28}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_FortunaCrane_Check_1.IdealTankMultiPort alt_fuel_tank(m_content_max = idealTank_TankcontentAltFuel, m_content_start = idealTank_TankcontentAltFuelStart, redeclare replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {70, -70}, extent = {{-10, -10}, {10, 10}})));
  // Part Renewable
  ...
  Tank tank2(redeclare replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.compressedhydrogen, SOC_start = H2SOCstart, V = H2Tankstorage, T(displayUnit = "K"), use_pressure_constraint = true) annotation(
    Placement(transformation(origin = {70, 142}, extent = {{-10, -10}, {10, 10}})));
  Tank tank3(redeclare replaceable package Medium = AZEAT_FortunaCrane_Check_1.Fuel.compressedhydrogen, SOC_start = H2SOCstart, V = H2Tankstorage, T(displayUnit = "K"), use_pressure_constraint = true) annotation(
  ...
  // Part Controller
  AZEAT_FortunaCrane_Check_1.MasterControllerSingleBattery masterControllerSingleBattery(
    n = N_gen, 
    alt_n = N_alt_gen, 
...
equation
  ...
  annotation(
    ...
    __OpenModelica_commandLineOptions = "--matchingAlgorithm=PFPlusExt --indexReductionMethod=dynamicStateSelection -d=NLSanalyticJacobian",
    __OpenModelica_simulationFlags(lv = "LOG_STDOUT,LOG_ASSERT,LOG_STATS", s = "dassl", variableFilter = ".*"));
end AZEAT_FortunaCrane_Check_1;
